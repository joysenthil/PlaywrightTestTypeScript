...
//create PUT API request body
 const putAPResponse = await request.put(`/booking/${bookingId}`, {
    data: putAPIRequestData,
    headers: {
...
expect(putAPResponse.status()).toBe(200);
expect(putAPResponse.statusText()).toBe('OK');
expect(putAPResponse.headers()['content-type']).toContain('application/json');
const jsonPutAPIResponse = await putAPResponse.json();
console.log('PutAPIResponse :' + JSON.stringify(jsonPutAPIResponse, null, 2));
